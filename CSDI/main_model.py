...
class CSDI_base(nn.Module):
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples, cond_fn=None):
        """
        Reverse diffusion sampling with optional graph guidance via cond_fn.
        NOTE: Guidance is applied *before* the score prediction at each t,
              so that the model's cross-feature coupling propagates changes
              to non-guided features naturally.
        """
        B, K, L = observed_data.shape
        ...
        with torch.no_grad():
            for i in range(n_samples):
                # for unconditional: build history of noisy conditioned observations
                if self.is_unconditional:
                ...
                for t in range(self.num_steps - 1, -1, -1):
                    # --- KEY: apply guidance BEFORE scoring ---
                    if cond_fn is not None:
                        current_sample = cond_fn(current_sample, cond_mask, t)

                    # build input to the diffusion model
                    if self.is_unconditional:
                    ...
                    # predict noise
                    eps_pred = self.diffmodel(diff_input, side_info, torch.tensor([t], device=device))

                    # DDPM update
                    coeff1 = 1 / self.alpha_hat[t] ** 0.5
                    coeff2 = (1 - self.alpha_hat[t]) / (1 - self.alpha[t]) ** 0.5
                    current_sample = coeff1 * (current_sample - coeff2 * eps_pred)
    ...
    def evaluate(self, batch, n_samples, cond_fn=None):
        (
            observed_data,
            observed_mask,
            observed_tp,
        ...
        with torch.no_grad():
            ...
            target_mask = observed_mask - cond_mask
            side_info = self.get_side_info(observed_tp, cond_mask)
            samples = self.impute(observed_data, cond_mask, side_info, n_samples, cond_fn=cond_fn)
...
class CSDI_Forecasting(CSDI_base):
    ...
    def process_data(self, batch):
        ...
        feature_id = torch.arange(self.target_dim_base).unsqueeze(0).expand(
